#!/usr/bin/env python3
"""N22 A3 occupied-slot hash emit. Optional --stack with N21 listed insert/rebuild.

Identity vs E6s control + T=0.3 VOI, then four-T. Not HASH_INSERT_ONLY.
...
def extra_env(slot: bool, insert: bool, rebuild: bool, four_tag: str, skip_build: bool):
    e = {
        "WATERZ_SLOT_EMIT": "1" if slot else "0",
...
def run_four(tag: str, slot: bool, insert: bool, rebuild: bool):
    env = parks_env(extra_env(slot, insert, rebuild, tag, True))
...
def run_216(tag: str, slot: bool, insert: bool, rebuild: bool):
...
def write_notes(tag: str, claim: str, doc: dict, slot: bool, insert: bool, rebuild: bool):
    lines = [
        f"# {tag} occupied-slot emit" + (" + listed stack" if insert or rebuild else ""),
        "",
        claim + ".",
        "",
        f"- env SLOT_EMIT={int(slot)} LISTED_INSERT={int(insert)} "
        f"LISTED_REBUILD={int(rebuild)}; FUSE_PACK unset; HASH_INSERT_ONLY unset; "
...
def main():
    ...
    if "--control-dump" in sys.argv:
        tag = "N22_A5" if stack else "N22_A3"
    ...
    rebuild = bool(stack)
    claim = (
        f"{tag} occupied-slot emit"
        + (" + listed insert/rebuild" if stack else "")
    ...
    os.environ["WATERZ_SLOT_EMIT"] = "1"
    os.environ["WATERZ_LISTED_INSERT"] = "1" if insert else "0"
    os.environ["WATERZ_LISTED_REBUILD"] = "1" if rebuild else "0"
    compile_d()
    lib = bind_parhac(ctypes.CDLL(str(DSO)))
    u, v, sm, ct, _fr, max_id = load_rag()
    ...
    trial = {
        ...
        "m2": m2,
        "env": (
            f"SLOT_EMIT=1 LISTED_INSERT={int(insert)} LISTED_REBUILD={int(rebuild)}"
        ),
    }
    vs_i0 = bool(
    ...
    if ctrl_npy.is_file():
        ctrl_npy.unlink()
    env = parks_env(extra_env(False, False, False, tag, True))
    env.pop("WATERZ_HASH_INSERT_ONLY", None)
    env.pop("WATERZ_FUSE_PACK", None)
    argv = [sys.executable, "-u", str(ROOT / "scripts/n22_a3.py"), "--control-dump"]
    if stack:
        argv.append("--stack")
    cr = subprocess.run(
    ...
    stamp_reason = None
    closer = None
    note = "occupied-slot emit" + (" + listed stack" if stack else "")
    if not identity:
        ...
        four = run_four(tag, slot, insert, rebuild)
        ...
        if not four_ok:
            ...
            if not doc.get("timing_usable_vs_1679"):
                ...
                closer = False
                stamp(tag, "timing_contaminated", {
                    "val_cut_vs_d0_ms": val_cut, "wall_ms": wall,
                }, note=note + "; do not 2.16")
            elif val_cut >= CUT_216_MS:
                run216 = run_216(tag, slot, insert, rebuild)
                doc["ran_216"] = True
                ...
                stamp_reason = "val_cut_ge_50_lt_100_no_216"
                closer = False
                stamp(tag, "val_cut_ge_50_lt_100_no_216", {
                    "val_cut_vs_d0_ms": val_cut, "wall_ms": wall,
                    "val_pin_ms": VAL_PIN_MS,
                    "identity": True, "four_pass": True, "ran_216": False,
                }, note=note + "; no 2.16")
    ...
    write_notes(tag, claim, doc, slot, insert, rebuild)
    print(json.dumps({
        "tag": tag,
        "identity": identity,
        "voi_ok": voi_ok,
