...
def main():
    cmd = [
        str(PY), str(ROOT / "src/segment.py"),
        str(AFF), "--out-dir", str(OUT),
    ]
    ...
    if "ACCURACY GATE: PASS" not in g.stdout:
        ...
        raise SystemExit(1)
    print("E11 G4r PASS")
    det = [str(PY), str(ROOT / "scripts/e3_g5.py"), "--eps", "0.08"]
    print("+", " ".join(det), flush=True)
    d = subprocess.run(det, cwd=str(ROOT))
    if d.returncode != 0:
        print("E11 G5r FAIL")
        raise SystemExit(1)
    print("E11 G5r PASS")
    ...
    import h5py
    import numpy as np
    from segment import _as_u8, _parhac, _rag, _watershed
    from ref_cpu import extract_parent
    ...
    print(
        f"E11 G6r one-shot ws={tws:.3f} rag={trag:.3f} agg={tagg:.3f} "
