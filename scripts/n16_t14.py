#!/usr/bin/env python3
"""N16 T14 SortPairs(keys, packed corner|vc). WATERZ_SORT_PACK=1 + T2+T5."""
...
NOTE = ROOT / "notes/N16_T14.md"
OUT = CACHE / "n16_t14.json"
...
def main():
    print("N16 T14 sort-pack. Not a 2 Gvox/s claim. Not 3090 Ti.", flush=True)
    busy = card_busy()
    if busy:
        print(f"N16 T14 REFUSE card busy: {busy}", flush=True)
        return 2
    ...
    print(f"N16 T14 ident {ident} stderr_tail={idr.stderr[-500:]}", flush=True)
    ...
    NOTE.write_text(
        "# N16 T14 sort-pack\n\n"
        "Not a 2 Gvox/s claim. Not 3090 Ti. WATERZ_SORT_PACK=1 + T2+T5.\n\n"
    ...
    print(f"N16 T14 keep={keep} ident={ok} ws={ws:.1f} -> {OUT}", flush=True)
