#!/usr/bin/env python3
"""N17 T1: FUSE_PACK on N16 deep stack. Never stacked in N16."""
...
NOTE = ROOT / "notes/N17_T1.md"
OUT = CACHE / "n17_t1.json"
EXTRA = {**DEEP, "WATERZ_FUSE_PACK": "1"}


def main():
    rc, doc = run_stack("T1 fuse_pack+deep", EXTRA, "n17_t1_four")
    ...
    NOTE.write_text(
        ...
        "N16 T15 ran FUSE_PACK alone; this stacks it on T2+T5+T6+T7.\n\n"
        ...
        f"vs deep agg {doc['deep_agg']:.1f}\n"
        f"- keep_default={doc['keep_default']}\n"
    )
    print(f"N17 T1 keep={doc['keep_default']} agg={doc['agg_ms']:.1f} -> {OUT}",
          flush=True)
    return rc
