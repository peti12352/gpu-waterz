#!/usr/bin/env python3
"""N15 exp9: WATERZ_MAX_OUTER=32 on E6s. ε stays 0.40.

Skip empty outers already exists (nabove==0). This tests a lower cap.
...
NOTE = ROOT / "notes/N15_T9.md"
OUT = CACHE / "n15_t9.json"
EXTRA = {"WATERZ_MAX_OUTER": "32"}


def main():
    print("N15 T9 max_outer=32. Not a 2 Gvox/s claim. Not 3090 Ti.", flush=True)
    busy = card_busy()
    if busy:
        print(f"N15 T9 REFUSE card busy: {busy}", flush=True)
        return 2
    ...
    print(f"N15 T9 voi {voi} stderr_tail={vr.stderr[-800:]}", flush=True)
    ...
    if voi_ok:
        ...
        print("N15 T9 skip 2.16 (VOI or 2-run failed)", flush=True)
    ...
    print(f"N15 T9 keep_default={keep} voi={voi_ok} agg={agg:.1f} -> {OUT}",
          flush=True)
