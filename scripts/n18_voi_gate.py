...
Env WATERZ_ABORT_SEC (default 120 val / 600 for 2.16) kills hangs.
"""
...
def abort_sec(kind: str) -> int:
    ...
    if env is not None and env.strip() != "":
        return int(env)
    return 600 if kind == "216" else 120


def run_cap(argv, env, timeout, label: str):
    """Subprocess with hard wall. Returns (doc, hang:bool, rc)."""
    try:
        r = subprocess.run(
...
def run_gate(
    ...
    """VOI-first gate. mode=voi_only|ident.

    Ship (keep_default) when: four-T PASS + VOI 2-run + det + timing gates.
    Identity vs wz_fragments.npy is diagnostic only in voi_only.
    """
    print(
        f"N18 {name} mode={mode}. {CLAIM}. Parks off. card_busy refuse.",
        flush=True,
    )
    busy = card_busy()
    if busy:
        print(f"N18 {name} REFUSE card busy: {busy}", flush=True)
    ...
    # Identity diagnostic (always collected when mode needs WS build artifacts)
    ident, ident_ok = {}, True
    if mode == "ident" or os.environ.get("WATERZ_N18_IDENT_DIAG", "1") == "1":
        ident, h, _ = run_cap(
            [sys.executable, str(ROOT / "scripts/n15_gate.py")],
            env, abort_sec("val"), "ident",
        )
        hang = hang or h
        print(f"N18 {name} ident_diag {ident}", flush=True)
        if mode == "ident":
            ident_ok = bool(
    ...
    if ident_ok and not hang:
        ...
        hang = hang or h
        print(f"N18 {name} voi {voi}", flush=True)
    ...
    if voi_ok and not hang:
        ...
        hang = hang or h
        print(f"N18 {name} four {four}", flush=True)
    ...
    if run_216 and four_ok and not hang:
        ...
        # Never reuse a prior 2.16 result after a failed/OOM run.
        if p216.is_file():
            p216.unlink()
        try:
            r = subprocess.run(
            ...
            rc216 = -9
            stamp(
                os.environ.get("WATERZ_N18_EXP", name),
                "hang",
    ...
    # N18 keep: VOI+four+det; WS lever ≤2149; agg lever ≤1862 (same numeric as N15 gates)
    keep = bool(
...
def main():
    ...
    ap.add_argument("--mode", choices=("voi_only", "ident"), default="voi_only")
    ap.add_argument("--name", default="n18_gate")
    ap.add_argument("--four-tag", default="n18_four")
    ...
    ap.add_argument(
        "--extra-json",
        default="",
        help='JSON object merged into DEEP env, e.g. {"WATERZ_VCOUNT_COMPACT":"1"}',
    )
    args = ap.parse_args()

    exp_id = args.exp_id or args.name
    os.environ["WATERZ_N18_EXP"] = exp_id
    ...
    rc, doc = run_gate(
        ...
        run_216=not args.no_216,
    )
    out = Path(args.out) if args.out else CACHE / f"{args.name}.json"
    CACHE.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(doc, indent=2) + "\n")
    print(json.dumps({"rc": rc, "out": str(out), "keep_default": doc.get("keep_default"),
                      "ws_ms": doc.get("ws_ms"), "agg_ms": doc.get("agg_ms"),
                      "e2e_ms": doc.get("e2e_ms"), "hang": doc.get("hang"),
                      "claim": CLAIM}), flush=True)
    return rc
