#!/usr/bin/env python3
"""N14 T1: fold k_uf_compress_c into consumers. WATERZ_FOLD_FLATTEN=1.
...
from n13_baseline import ident, load_json_line, parks_env  # noqa: E402
from p1_make_big_indep import CACHE, card_busy, gpu_state  # noqa: E402
from b_dev_aff import build  # noqa: E402

NOTE = ROOT / "notes/N14_T1.md"
OUT = CACHE / "n14_t1.json"
WS_BASE = 2579.0453841909766
GATE = WS_BASE / 1.2


def main():
    print("N14 T1 fold flatten. Not a 2 Gvox/s claim. Not 3090 Ti.", flush=True)
    busy = card_busy()
    if busy:
        print(f"N14 T1 REFUSE card busy: {busy}", flush=True)
        return 2
    ...
    print(f"N14 T1 ident {ident_doc} stderr_tail={idr.stderr[-500:]}", flush=True)
    r216 = subprocess.run(
    ...
    NOTE.write_text(
        "# N14 T1 fold flatten\n\n"
        "Not a 2 Gvox/s claim. Not 3090 Ti. WATERZ_FOLD_FLATTEN=1. "
        "Default UF=3 unchanged unless keep_default.\n\n"
    ...
    print(f"N14 T1 keep_default={keep} ws={ws:.1f} -> {OUT}", flush=True)
