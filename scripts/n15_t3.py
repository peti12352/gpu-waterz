#!/usr/bin/env python3
"""N15 exp3: final k_uf_jump flatten (not skip, not UF_ALGO=1).

WATERZ_JUMP_FLATTEN=1. Default fold off. Identity then 2.16 WS vs 2579/1.2.
...
NOTE = ROOT / "notes/N15_T3.md"
OUT = CACHE / "n15_t3.json"
EXTRA = {"WATERZ_JUMP_FLATTEN": "1"}


def main():
    print("N15 T3 jump flatten. Not a 2 Gvox/s claim. Not 3090 Ti.", flush=True)
    busy = card_busy()
    if busy:
        print(f"N15 T3 REFUSE card busy: {busy}", flush=True)
        return 2
    ...
    print(f"N15 T3 ident {ident_doc} stderr_tail={idr.stderr[-800:]}", flush=True)
    ...
    if ident_ok and det_ok:
        ...
        print("N15 T3 skip 2.16 (identity or 2-run failed)", flush=True)
    ...
    NOTE.write_text(
        ...
        "Not a 2 Gvox/s claim. Not 3090 Ti. WATERZ_JUMP_FLATTEN=1. "
        "Not UF_ALGO=1. Not fold skip.\n\n"
    ...
    print(f"N15 T3 keep_default={keep} ident={ident_ok} ws={ws:.1f} -> {OUT}",
          flush=True)
