#!/usr/bin/env python3
"""L33: SDSL then exact S4 only on leftover live S3>T. Lock iff PASS and residual<=5k."""
...
def main():
    ...
    lib.leftover_s4_cpu.restype = ctypes.c_int
    lib.leftover_s4_cpu.argtypes = [
    ...
    print(f"L33 S0={args.s0}", flush=True)
    t0 = time.time()
    rc = lib.leftover_s4_cpu(
    ...
    print(f"L33 rc={rc} wall={wall:.3f} residual={list(map(int, residual))} max={nres}", flush=True)
    if rc != 1:
        stamp("l33", False, f"rc={rc}")
        return False
    buf = io.StringIO()
    with redirect_stdout(buf):
        ok = grade_parents(parents, fr, "L33", f"l33_s{int(args.s0)}")
    sys.stdout.write(buf.getvalue())
    if ok and nres <= 5000:
        stamp("l33", True, f"LOCK leftover n_residual={nres} S0={args.s0} wall={wall:.3f}")
        print("L33 PASS LOCK", flush=True)
        return True
    if ok and nres < 50_000:
        stamp("l33", False, f"PASS-serial / FAIL-depth n_residual={nres} S0={args.s0}")
        print("L33 PASS-serial / FAIL-depth (Q20-class, no GPU heap)", flush=True)
        return False
    stamp("l33", False, f"{'PASS' if ok else 'FAIL'} n_residual={nres} S0={args.s0}")
    print(f"L33 {'PASS' if ok else 'FAIL'} residual={nres}", flush=True)
    return False
