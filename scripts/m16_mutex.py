#!/usr/bin/env python3
"""M16: Wolf Alg. 2 / GASP AbsMax on signed RAG. Attractive=mean, repulsive=1-mean."""
...
def main():
    ...
    print(f"M16 edges={len(u)} max_id={max_id}", flush=True)
    ...
    wall = time.time() - t0
    print(f"M16 rc={rc} wall={wall:.3f}", flush=True)
    if rc != 1:
        raise SystemExit(2)
    dest_dir = OUT / "m16_mutex"
    ...
    for i, thr in enumerate(AFF_THRESHOLDS):
        ...
        print(f"  T={thr} nseg={nseg} merges={int(stats[i,1])} mutex={int(stats[i,2])}", flush=True)
    ...
    print(f"M16 {'PASS' if ok else 'FAIL'} wall={wall:.3f}")
    if not ok:
        raise SystemExit(1)
