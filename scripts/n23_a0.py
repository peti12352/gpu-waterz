#!/usr/bin/env python3
"""N23 A0: A5 stack (SLOT_EMIT+LISTED_INSERT+LISTED_REBUILD) forced 2.16.

Reconfirm val identity + four-T, then 2.16 even if val cut < 100 ms.
A5 flags stay default off. Idle RTX 5090.
"""
...
CLAIM = "N23_A0 A5 stack forced 2.16; idle RTX 5090; flags stay default off"
...
def legal_env():
    ...
    os.environ.pop("WATERZ_CSR_REWRITE", None)


def extra_env(a5: bool, four_tag: str, skip_build: bool):
    e = {
        "WATERZ_SLOT_EMIT": "1" if a5 else "0",
        "WATERZ_LISTED_INSERT": "1" if a5 else "0",
        "WATERZ_LISTED_REBUILD": "1" if a5 else "0",
...
def main():
    ...
    trial = {
        ...
        "m1": m1,
        "m2": m2,
        "env": "SLOT_EMIT=1 LISTED_INSERT=1 LISTED_REBUILD=1",
    }
    vs_i0 = bool(
    ...
    cr = subprocess.run(
        [sys.executable, "-u", str(ROOT / "scripts/n23_a0.py"), "--control-dump"],
    ...
    doc = {
        ...
        "ran_216": False,
        "keep_default": False,
        "csr_exception_100ms": "A0 bypasses val 100 ms gate; A5 flags still default off",
    }
                ...
                if run216.get("rc216") not in (0, None) or agg <= 0:
                    ...
                    if agg2 <= 0 or agg2 >= PIN_AGG_216 - NOISE_MS:
                        ...
                        stamp(TAG, "216_cut_not_repro", {
                            ...
                            "pin": PIN_AGG_216,
                        }, note="A5 stack; second 2.16 did not hold 50 ms")
                    else:
                        stamp(TAG, "216_cut", {
                            "agg_216_ms": agg, "agg_216_ms_2": agg2,
                            "pin": PIN_AGG_216, "e2e_ms": e2e,
                        }, note="A5 stack; keep_default false")
                    ...
                    stamp(TAG, "216_no_cut", {
                        "agg_216_ms": agg, "pin": PIN_AGG_216, "e2e_ms": e2e,
                    }, note="A5 stack forced 2.16; not a closer")
