#!/usr/bin/env python3
"""N15 exp4: skip flatten only on e9b (WATERZ_E9B_FOLD_ONLY=1).

e9c still k_uf_compress_c; e9c write has no find. Share stays on.
Identity then 2.16. Not a 2 Gvox/s claim. Not 3090 Ti.
"""
...
NOTE = ROOT / "notes/N15_T4.md"
OUT = CACHE / "n15_t4.json"
EXTRA = {"WATERZ_FOLD_FLATTEN": "1", "WATERZ_E9B_FOLD_ONLY": "1"}


def main():
    print("N15 T4 e9b-only fold. Not a 2 Gvox/s claim. Not 3090 Ti.", flush=True)
    busy = card_busy()
    if busy:
        print(f"N15 T4 REFUSE card busy: {busy}", flush=True)
        return 2
    ...
    print(f"N15 T4 ident {ident_doc} stderr_tail={idr.stderr[-800:]}", flush=True)
    ...
    if ident_ok and det_ok:
        ...
        print("N15 T4 skip 2.16 (identity or 2-run failed)", flush=True)
    ...
    NOTE.write_text(
        ...
        "WATERZ_FOLD_FLATTEN=1 WATERZ_E9B_FOLD_ONLY=1. Share on.\n\n"
    ...
    print(f"N15 T4 keep_default={keep} ident={ident_ok} ws={ws:.1f} -> {OUT}",
          flush=True)
