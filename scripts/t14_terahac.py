#!/usr/bin/env python3
"""T14: TeraHAC control flow + S3 contact-mean. Default ε=0.1 official."""
...
def main():
    ...
    p.add_argument("--cap", type=int, default=0, help="size_constraint; 0=official max(n/100,1e6)")
    args = p.parse_args()
    print_contract()
    compile_so()
    u, v, sm, ct = load_rag()
    ...
    print(f"T14 edges={len(u)} max_id={max_id} eps={args.eps} cap={args.cap}", flush=True)
    ...
    lib.terahac_mean_cpu.restype = ctypes.c_int
    lib.terahac_mean_cpu.argtypes = [
    ...
    rc = lib.terahac_mean_cpu(
        ...
        ctypes.c_int64(args.cap),
    )
    wall = time.time() - t0
    print(f"T14 rc={rc} wall={wall:.3f}", flush=True)
    if rc != 1:
        raise SystemExit(2)
    dest_dir = OUT / f"t14_eps{args.eps}_cap{args.cap}"
    ...
    for i, thr in enumerate(AFF_THRESHOLDS):
        ...
        outers.append(outer)
        print(
            f"  T={thr} nseg={nseg} outer={outer} merges={int(stats[i,1])} "
            f"red_viol={int(stats[i,2])}",
            flush=True,
        )
    ...
    mx = max(outers) if outers else -1
    print(f"T14 eps={args.eps} cap={args.cap} {'PASS' if ok else 'FAIL'} "
          f"max_outer={mx} wall={wall:.3f}")
    if not ok:
        raise SystemExit(1)
    if mx > 80:
