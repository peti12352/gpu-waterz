#!/usr/bin/env bash
# Remaining N20 CPU jobs after D1/D2. Skip D2 redo (height already kills chain).
# Wall vs 1679.9 only if n20_res says idle. GPU prep never launches.
set -u
...
LOG="${ROOT}/notes/N20_CPU_REST.log"
...
{
  echo "host=$(hostname) date=$(date -Is) rest_campaign"
  echo "N20_GPU_RUN=${N20_GPU_RUN}"
  "$PY" -u scripts/n20_res.py snap
  wait_idle UNIT
  taskset -c 2 "$PY" -u scripts/n20_unit.py
  echo "n20_unit exit=$?"
  wait_idle N20_X4
