...
by >=1.95x (the honest-stack gate without a proven make_big reflect). The
only extra T=0.3-legal V is ε=0.40 (1.04x on top of E1's ε=0.32). That
...
|---|---|---|---|
| leftover -> WS (honest) | 1642 ms | 1.315 | 1.52x |
| leftover -> alloc (old E6) | 3692 ms | 0.585 | 3.42x |
| leftover -> sync | 1738 ms | 1.242 | 1.61x |

Honest stack 3: ws 227 + rag 192 + agg 1150 + extract 73 = 1642 ms.
Agg must be ≤ 587 ms -> **another 1.96x on 1150**.
Even WS+RAG reflect (N4, unproven on real aff) leaves agg ≤ 1025 ms -> 1.12x.
...
|---|---|---|---|---|---|
| 0.32 (E1) | 0.4451 | 0.2509 | 933245065 | 2.88x | PASS |
| **0.40** | 0.4408 | 0.2543 | 894217121 | **3.01x** | **PASS** |
| 0.48 | 0.4422 | 0.2892 | 723337741 | 3.72x | FAIL merge 0.2892>0.2611 |

Best legal V is now ε=0.40 size_asym on, T=0.3 only. Extra vs E1: 1.04x.
Replacing 1150 ms with 1150x2.88/3.01 = 1100 ms still 1.87x the 587 ms
agg budget.
...
- prefilter edge cut 1.09x (97% of edges are already above T=0.3)
- mean degree 6.90; locked visits/merge ~1152 vs essential ~7 (167x slack
  is the G2 scan, already counted in E2's 3.9x: not a new 167x)
- prefilter mean≤T drop: fingerprint != locked; sum_nlive cut 1.19x;
  T=0.3 VOI PASS (0.4527 / 0.2513). Not 1.95x. Not stackable on E2
...
This is not a 12x wall-clock claim. Time is still val e2e x (1792/1008).
make_big.py and affinity.h5 were not on disk; no 2-tile RAG identity on
the real generator output. N5 therefore does **not** take the 1.18x gate.
