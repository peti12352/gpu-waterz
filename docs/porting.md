...
is not. HBM GB/s is the wrong scaler for these kernels.
...
measure peak on the card you run before changing buffer-sharing.

---

## 3. How these milliseconds were measured
...
| kernel | ms | note |
|---|---|---|
| `k_w5_compress_list` | 508 | WS; 71-105 M list entries, including plateau faces. Jump-4 was slower |
...
large (~96 MB); several gathers are cache-sensitive.

Leftover if the heavy agg kernels vanished: still ~690-1130 ms agg plus
hundreds of ms of WS. Mutex, Kruskal, RNN, and StarMerge change the
partition or do not close 1680 ms. See [lab.md](../notes/lab.md).
